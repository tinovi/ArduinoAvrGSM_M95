...
#define __TOUTBEGINWRITE__ 90000
...
// 1 if success
// >1 if error 
int GSM3MobileClientService::ready(char c)
{	
...
int GSM3MobileClientService::waitForAnswer(char c)
{
	int res;
	for(int i = 0;i<300 &&ready(c)==0;i++){
		delay(300);
	}
...
void GSM3MobileClientService::stop()
{
	if(flags & GSM3MOBILECLIENTSERVICE_WRITING)
		endWrite(true);
	if(theGSM3MobileClientProvider->disconnectTCP(flags & GSM3MOBILECLIENTSERVICE_CLIENT, mySocket)){
