...
bool GSM3ShieldV1DataNetworkProvider::connected(){
    String modemResponse=modem.writeModemCommand("AT+CGATT?", 1000);
	if(modemResponse.indexOf("+CGATT: 1")>=0)
      return true;
	else
		return false;
}
...
void GSM3ShieldV1DataNetworkProvider::attachGPRSContinue()
{
	...
	// 5: Wait for disable MUXIP OK and Set the session mode as transparent "AT+QIMODE=0"
...
//				theGSM3ShieldV1ModemCore.setCommandCounter(13);
				...
				theGSM3ShieldV1ModemCore.genericCommand_rq(PSTR("AT+QICSGP=1"));
				theGSM3ShieldV1ModemCore.setCommandCounter(13);
			}
			else theGSM3ShieldV1ModemCore.closeCommand(3);
		}
...
IPAddress GSM3ShieldV1DataNetworkProvider::getIPAddress() {
	...
	while((millis()-m)<10*1000 && (!ready('I'))){
