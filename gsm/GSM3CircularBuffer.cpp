...
//void GSM3CircularBuffer::printRX(uint8_t c)
//{
//	Tr('|');
//	if((c>31)&&(c<127))
//		Tr((char)c);
...
char GSM3CircularBuffer::peek(int increment)
{
	...
	else num_aux = __BUFFERSIZE__ - head + tail;
...
void GSM3CircularBuffer::flush(uint8_t until){
	head=until;
}
...
//DEBUG
//Trln("extrSs");
//Tr("h,t=");Tr(int(head));Tr(",");Trln(int(tail));
	
	if(!locate(from, head, tail, 0, &t1))
		return false;
		
//DEBUG
//Trln("Located chain from.");
